...
// Scroll lock: lock <html>, compensate scrollbar width on <body>.
// Does NOT use position:fixed on body (causes layout jump + iOS snap-to-top).
// Does NOT use overflow:hidden on body alone (doesn't stop iOS rubber-band).
// Locks <html> with overflow:hidden — no BFC creation, zero repaint, zero flash.
function lockScroll() {
...
function unlockScroll() {
  const html = document.documentElement
  const scrollY = parseInt(html.dataset.scrollY || '0', 10)
  html.style.overflow = ''
...
export default function PhotoModal({ photo: initialPhoto, onClose, onPhotoUpdated, onPhotoDeleted }: Props) {
  ...
  // Sync photo state when prop changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (!initialPhoto || !isAuthenticated || favLoaded) return
    fetch('/api/favourites')
      .then(r => r.ok ? r.json() : { photoIds: [] })
      .then(({ photoIds }) => {
        setIsFavourited((photoIds as string[]).includes(initialPhoto.id))
        setFavLoaded(true)
      })
      .catch(() => setFavLoaded(true))
  }, [initialPhoto?.id, isAuthenticated, favLoaded]) // eslint-disable-line react-hooks/exhaustive-deps

  // Scroll lock — html overflow:hidden, body padding-right compensates scrollbar
  useEffect(() => {
  ...
  // Prevent iOS rubber-band scroll bleeding through to the page behind modal
  useEffect(() => {
    if (!initialPhoto) return
  ...
  }, [photo, isFavourited, togglingFav])
  ...
  return (
    ...
    >
      ...
      <div className="modal-card relative bg-stone-50 w-full max-w-4xl mx-4 my-8 rounded-sm shadow-2xl animate-scaleIn">
        ...
        <div className="px-6 pb-6 lg:px-8 lg:pb-8">
          {saveSuccess && (
            <div className="mb-4 p-3 bg-stone-900 text-white text-sm lowercase rounded animate-slideDown">
          ...
          {/* Photo */}
          <div
            className="relative w-full overflow-hidden rounded-sm mb-6 bg-stone-100"
          ...
          >
            ...
            <img
              src={photo.image_url}
              alt={photo.story}
              className="w-full h-auto block max-h-[70vh] object-contain bg-stone-100"
            />
              ...
              <div className="space-y-5 animate-fadeIn">
                ...
                <div className="flex gap-3 pt-1">
                  <button
                    ...
                    className="px-6 py-2 bg-stone-900 text-white text-sm lowercase tracking-wide rounded hover:bg-stone-800 disabled:opacity-50 transition-colors duration-150"
                  >
                    {saving ? 'saving...' : 'save'}
                  </button>
                  <button
                    type="button" onClick={() => { setIsEditing(false); setSaveError('') }} disabled={saving}
      ...
      {showDeleteModal && (
        <div
          ...
          onClick={(e) => { if (e.currentTarget === e.target) setShowDeleteModal(false) }}
        >
          <div className="bg-white rounded-lg shadow-2xl p-8 max-w-sm w-full mx-4 animate-scaleIn">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm lowercase tracking-wide rounded disabled:opacity-50 transition-colors duration-150"
              >
